...
namespace TextEncoding
{
    Encoding detectEncoding(const juce::MemoryBlock& data)
    {
        ...
        while (i < size)
        {
            ...
            {
                if (b < 0xC2) { isValidUTF8 = false; break; } // overlong
                continuationBytes = 1;
            }
            else if ((b & 0xF0) == 0xE0)
            {
            ...
            {
                if (b > 0xF4) { isValidUTF8 = false; break; }
                continuationBytes = 3;
            }
            else
            {
    ...
    juce::String shiftJisToString(const void* data, size_t numBytes)
    {
...
#if JUCE_WINDOWS
        int wideLen = MultiByteToWideChar(932, 0,
                                          static_cast<LPCCH>(data),
        ...
        CFStringRef cfStr = CFStringCreateWithBytes(
            ...
            static_cast<CFIndex>(numBytes),
            kCFStringEncodingDOSJapanese,
            false);
        if (cfStr == nullptr)
            return {};
        CFIndex utf8Len = CFStringGetMaximumSizeForEncoding(
